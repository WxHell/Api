...
const queryBuilder = async (Model, req, options = {}) => {
  try {
    ...
    const defaults = {
      defaultLimit: 15, // Varsayılan sayfa boyutu
    ...
    if (req.query.category) {
      ...
      if (!category) {
        // ❗ res kullanmadan doğrudan boş veri dönelim
        filterObject["categoryId"] = null;
      } else {
        // Eğer varsa filtre uygula
        filterObject["categoryId"] = { $in: [category._id] };
      }
    }
    ...
    if (filterToProcess && typeof filterToProcess === "object") {
      Object.keys(filterToProcess).forEach((field) => {
        ...
        if (
          config.allowedFilterFields.length === 0 ||
